...
export default function Carrocarias({ content, openQuote }) {
  return (
    <main>
      <section style={s("position:relative;background:#0F0F11;overflow:hidden;")}>
        <img src="https://loremflickr.com/1400/800/truck,factory?lock=12" alt="" style={s("position:absolute;inset:0;width:100%;height:100%;object-fit:cover;")} />
        ...
        <div style={s("position:relative;max-width:1240px;margin:0 auto;padding:84px 32px 90px;")}>
          ...
          <p style={s("font-family:'Barlow',sans-serif;font-size:19px;line-height:1.55;color:#C7C7CC;max-width:56ch;margin:24px 0 0;")}>Fabricamos e reparamos a carroçaria certa para cada veículo de trabalho. Escolha o tipo de solução e peça-nos um orçamento à medida.</p>
        </div>
      ...
      <section style={s("max-width:1240px;margin:0 auto;padding:72px 32px 90px;")}>
        <div style={s("display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:24px;")}>
          {content.carrocarias.map((item) => (
            <El key={item.id || item.title} css="border:1px solid #E4E4E0;background:#fff;display:flex;flex-direction:column;overflow:hidden;" hover="border-color:#16161A;">
              ...
              <div style={s("padding:24px;display:flex;flex-direction:column;flex:1;")}>
                ...
                <El tag="button" onClick={() => openQuote("Carroçarias — " + item.title)}
                  css="margin-top:auto;align-self:flex-start;background:#16161A;border:none;cursor:pointer;font-family:'Oswald',sans-serif;font-weight:600;text-transform:uppercase;letter-spacing:.08em;font-size:13px;color:#fff;padding:12px 20px;"
                  hover="background:#E01E26;">Pedir orçamento</El>
              </div>
            </El>
          ))}
        </div>
